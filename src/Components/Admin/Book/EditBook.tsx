...
import {
  GetAuthorDto,
  GetPublisherDto,
} from "../../../routes/Admin/Books/BookInventoryListPage";
import { toast } from "react-toastify";
import { IoClose } from "react-icons/io5";
import { IoIosArrowDown } from "react-icons/io";
import { useParams } from "react-router";
import { API_BASE_URL } from "../../../Constants";
import { ValidateFileExentension, ValidateFileSize } from "./AddBook";
...
export const EditBook = (props: Props) => {
  const adminToken = getCookie("LMS");
  console.log(adminToken);
  const params = useParams();

  const editBookFormRef = useRef<HTMLFormElement>(null);
  const fileBookCoverInputRef = useRef<HTMLInputElement>(null);
  const [selectedAuthors, setSelectedAuthors] = useState<GetAuthorDto[]>([]);
  useEffect(() => {
    if (props.authors?.length > 0 && selectedAuthors.length === 0)
      setSelectedAuthors(props.authors);
  }, [props.authors]);
  const [selectedPublishers, setSelectedPublishers] = useState<
    GetPublisherDto[]
  >([]);
  useEffect(() => {
    if (props.publishers?.length > 0 && selectedPublishers.length === 0)
      setSelectedPublishers(props.publishers);
  }, [props.publishers]);
  const ValidateBookTitle = async (value: string) => {
    try {
      const adminToken = getCookie("LMS");
      const response = await fetch(
        `${API_BASE_URL}/Book/CheckBookTitleUniqueForUpdate/${params.bookId}?adminToken=${adminToken}&title=${value}`,
        {
          method: "GET",
        }
      );
  ...
  const editBookSchema = Yup.object().shape({
    ...
    bookName: Yup.string()
      ...
      .min(10, "Too Short!")
      .max(70, "Too Long!")
      .test("bookName", "Book Name is already exists !.", ValidateBookTitle),
  ...
  const formik = useFormik({
    initialValues: {
      ...
      genre: props.genre,
      cost: props.bookCost,
      borrowCost: props.bookBorrowCost,
      publishDate: props.publishDate,
      pages: props.noOfPages,
      currentStock: props.currentStock,
      actualStock: props.actualStock,
    ...
    onSubmit: async () => {
      try {
        ...
        const formData = new FormData(
          editBookFormRef.current !== null ? editBookFormRef.current : undefined
        );
        formData.append("adminToken", adminToken);
        formData.append("id", params.bookId !== undefined ? params.bookId : "");
        if (formik.values.bookCover !== null)
          formData.append("bookCover", formik.values.bookCover);
        ...
        formData.append("noOfPages", formik.values.pages.toString());
        formData.append("actualStock", formik.values.actualStock.toString());
        formData.append("publishDate", formik.values.publishDate);
        formData.append("currentStock", formik.values.currentStock.toString());
  ...
  // const editBookForm = useRef<HTMLFormElement>(null);
  const [editBookHeaderDivWidth, setAddBookHeaderDivWidth] = useState<number>(
    window.innerWidth
  );
  ...
  useEffect(() => {
    if (formik.isSubmitting && selectedAuthors.length === 0 && props.authors)
      formik.setFieldError("authorIds", "Please, select just one author!.");
  }, [selectedAuthors, formik, props.authors]);
  useEffect(() => {
    if (
      formik.isSubmitting &&
  ...
  }, [selectedPublishers, formik, props.publishers]);
  return (
    <form
      ref={editBookFormRef}
      onSubmit={(event) => {
      ...
      style={{
        width: `${editBookHeaderDivWidth * 0.7}px`,
        marginBottom: "50px",
      }}
    >
      <h2
      ...
      <div
        className={editBookHeaderDivWidth <= 1000 ? "h-[1870px]" : "h-[1250px]"}
        style={{
          width: "98%",
          paddingTop: "20px",
          marginLeft: "auto",
      ...
      >
        ...
        >
          <img
            ...
            src={"data:image/png;base64," + props.bookCover}
            width={"150px"}
            height={"150px"}
          />
        </div>
        <h3
          className="font-notoSansTeluguBold text-2xl"
          style={{
            textAlign: "center",
            margin: 0,
            marginBottom: "10px",
        ...
        <div className="relative z-0 hover:cursor-pointer w-[90%] mx-auto bg-transparent flex gap-x-2 items-center h-11 px-4 border-2 border-gray-600 rounded-sm">
          <h3 className="absolute left-0 bottom-full mb-2">
        ...
        <div className="flex justify-start w-[90%] mx-auto">
          {formik.touched.bookCover &&
            typeof formik.errors.bookCover === "string" && (
        ...
        <div className="relative w-[90%] mx-auto bg-white flex flex-col gap-y-[3%] mb-2">
          ...
          {allAuthors.length === 0 ? (
            ...
            <div className="z-20 absolute left-0 top-full h-42 w-full overflow-auto border-2 border-gray-600 rounded-sm pl-[5%]">
        ...
        <div className="relative w-[90%] mx-auto bg-white flex flex-col gap-y-[3%] mb-2">
          ...
          {allPublishers.length === 0 ? (
            ...
            <div className="absolute left-0 top-full h-42 w-full overflow-auto border-2 border-gray-600 rounded-sm pl-[5%]">
        ...
        >
          <button
            className="rounded-sm bg-yellow-500 hover:bg-yellow-600"
            type="submit"
            style={{
              width: "100%",
              height: "50px",
