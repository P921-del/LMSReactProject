...
interface Props {
  allAuthors: GetAuthorDto[];
  setAllAuthors: React.Dispatch<React.SetStateAction<GetAuthorDto[]>>;
}

export const AuthorInventoryList = (mainProps: Props) => {
  interface SubAuthorListProps {
    currentItems: GetAuthorDto[];
  }
  ...
  const SubAuthorList = (props: SubAuthorListProps) => {
    const [subAuthorListHeaderDivWidth, setSubAuthorListHeaderDivWidth] =
      useState<number>(window.innerWidth);
    const [WindowWidth, setWindowWidth] = useState<number>(window.innerWidth);
    useEffect(() => {
      function handleResizing() {
        setSubAuthorListHeaderDivWidth(window.innerWidth);
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            const response = await fetch(
              `${API_BASE_URL}/Author/delete/${authorId}?adminToken=${adminToken}`,
              {
                method: "PUT",
                headers: {
            ...
            if (!response.ok) {
              ...
              Swal.fire({
                ...
                text: "Author is deleted successfully",
                icon: "success",
                confirmButtonColor: "black",
              });
              try {
                const secondResponse = await fetch(
                  `${API_BASE_URL}/Author/authors`
                );
        ...
        style={
          WindowWidth >= 1000 && WindowWidth <= 1400
            ...
            : WindowWidth > 1400 && WindowWidth <= 2048
              ...
              : { width: `${subAuthorListHeaderDivWidth}px` }
        }
      >
        ...
        >
            ...
            style={
              WindowWidth >= 1000 && WindowWidth <= 1400
                ...
                : WindowWidth > 1400 && WindowWidth <= 2048
                  ...
                  : {
                      width: `${subAuthorListHeaderDivWidth * 0.8}px`,
                      marginInline: "auto",
                    }
            }
          >
          ...
          >
            {props.currentItems &&
              props.currentItems.map((item: GetAuthorDto) => (
                <div
                  key={item.id}
                ...
                >
                  ...
                  >
                    <span
                      onClick={() =>
                        navigate(`/admin/authors/update-author/${item.id}`)
                      }
  ...
  function PaginatedItems(props: PaginatedItemsProps) {
    ...
    const currentItems = mainProps.allAuthors.slice(itemOffset, endOffset);
    const pageCount = Math.ceil(
      mainProps.allAuthors.length / props.itemsPerPage
    );

    // Invoke when user click to request another page.
    const handlePageClick = (event: { selected: number }) => {
      const newOffset =
        (event.selected * props.itemsPerPage) % mainProps.allAuthors.length;
      console.log(
    ...
    return (
      <>
        <SubAuthorList currentItems={currentItems} />
