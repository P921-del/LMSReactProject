...
const AddBorrowedBookSchema = Yup.object().shape({
...
export const AddBorrowedBook = () => {
  const addBorrowedBookFormRef = useRef<HTMLFormElement>(null);
  ...
  const formik = useFormik({
    ...
    validationSchema: AddBorrowedBookSchema,
    onSubmit: async () => {
      try {
        const adminToken = getCookie("LMS");

        const formData = new FormData(
          addBorrowedBookFormRef.current !== null
            ? addBorrowedBookFormRef.current
            : undefined
        );
        ...
        const response = await fetch(`${API_BASE_URL}/BorrowBook/create`, {
          method: "POST",
          body: formData,
        });
        ...
        if (!response.ok) {
          ...
          Swal.fire({
            ...
            text: "BorrowedBook is added successfully",
            icon: "success",
  ...
  const [addBorrowedBookHeaderDivWidth, setAddBorrowedBookHeaderDivWidth] =
    useState<number>(window.innerWidth);
  useState<number>(window.innerWidth);
  useEffect(() => {
    function handleResizing() {
      setAddBorrowedBookHeaderDivWidth(window.innerWidth);
    }
  ...
  return (
    <form
      ...
      style={{
        width: `${addBorrowedBookHeaderDivWidth * 0.7}px`,
        marginBottom: "50px",
      }}
    >
      <h2
      ...
      <div
        className={
          addBorrowedBookHeaderDivWidth <= 1000 ? "h-[1000px]" : "h-[650px]"
        }
        style={{
          width: "98%",
          paddingTop: "20px",
